...
export function openMailAccountSetup(): void {
  ...
  title.textContent = t('Add Email Account');
  footer.style.display = '';
  footer.textContent = t('Connect Account');

  body.innerHTML = `
    <p class="channel-setup-desc">${t('Choose your email provider to get started.')}</p>
...
function showMailAccountForm(providerId: string): void {
  ...
  body.innerHTML = `
    <div class="mail-setup-back" id="mail-setup-back">← ${t('Choose provider')}</div>
    ${provider.hint ? `<div class="mail-setup-hint"><span class="ms ms-sm">info</span> ${t(provider.hint)}</div>` : ''}
    <div class="form-group">
      <label class="form-label" for="ch-field-mail-email">${t('Email Address')} <span class="required">*</span></label>
    ...
    <div class="form-group">
      <label class="form-label" for="ch-field-mail-display">${t('Display Name')}</label>
      <input class="form-input" id="ch-field-mail-display" type="text" placeholder="Your Name">
      <div class="form-hint">${t('How your name appears in outgoing emails')}</div>
    </div>
    <div class="form-group">
      <label class="form-label" for="ch-field-mail-password">${needsAppPw ? t('App Password') : t('Password')} <span class="required">*</span></label>
      <input class="form-input" id="ch-field-mail-password" type="password" placeholder="${providerId === 'gmail' ? t('16-character app password') : t('Password')}">
    </div>
    ${
      isCustom
        ? `
    <div class="form-row-2col">
      <div class="form-group">
        <label class="form-label" for="ch-field-mail-imap">${t('IMAP Server')} <span class="required">*</span></label>
        <input class="form-input" id="ch-field-mail-imap" type="text" placeholder="imap.example.com" value="${escAttr(provider.imap)}">
      </div>
      <div class="form-group">
        <label class="form-label" for="ch-field-mail-imap-port">${t('IMAP Port')}</label>
        <input class="form-input" id="ch-field-mail-imap-port" type="number" value="${provider.imapPort}">
      </div>
    </div>
    <div class="form-row-2col">
      <div class="form-group">
        <label class="form-label" for="ch-field-mail-smtp">${t('SMTP Server')} <span class="required">*</span></label>
        <input class="form-input" id="ch-field-mail-smtp" type="text" placeholder="smtp.example.com" value="${escAttr(provider.smtp)}">
      </div>
      <div class="form-group">
        <label class="form-label" for="ch-field-mail-smtp-port">${t('SMTP Port')}</label>
    ...
    <div class="mail-permissions-setup">
      <div class="mail-permissions-title">${t('Agent permissions')}</div>
      <div class="mail-permissions-desc">${t('Control what your agent can do with this account. You can change these any time from the Credential Vault.')}</div>
      <label class="mail-perm-toggle">
        <input type="checkbox" id="ch-field-perm-read" checked>
        <span class="mail-perm-label">${t('Read emails')}</span>
        <span class="mail-perm-detail">${t('List inbox, read messages, search')}</span>
      </label>
      <label class="mail-perm-toggle">
        <input type="checkbox" id="ch-field-perm-send" checked>
        <span class="mail-perm-label">${t('Send emails')}</span>
        <span class="mail-perm-detail">${t('Compose and send on your behalf')}</span>
      </label>
      <label class="mail-perm-toggle">
        <input type="checkbox" id="ch-field-perm-delete">
        <span class="mail-perm-label">${t('Delete emails')}</span>
        <span class="mail-perm-detail">${t('Move to trash, permanently delete')}</span>
      </label>
      <label class="mail-perm-toggle">
        <input type="checkbox" id="ch-field-perm-manage">
        <span class="mail-perm-label">${t('Manage folders')}</span>
        <span class="mail-perm-detail">${t('Create folders, move messages, flag')}</span>
      </label>
    </div>
    ...
    <div class="mail-security-info">
      <div class="mail-security-header">
        ...
        ${t('How your credentials are stored & used')}
      </div>
      <ul class="mail-security-list">
        <li><strong>${t('OS keychain')}</strong> — ${t('your password is stored in the system keychain (macOS Keychain / libsecret on Linux), not in any file.')}</li>
        <li><strong>${t('Never sent to frontend')}</strong> — ${t('credential details are redacted before reaching the UI.')}</li>
        <li><strong>${t('TLS in transit')}</strong> — ${t('connections to')} ${provider.imap || 'your mail server'} ${t('use TLS encryption')}</li>
        <li><strong>${t('No cloud')}</strong> — ${formatBrandText('{appName} is fully self-hosted, your data never leaves your machine')}</li>
        <li><strong>${t('Permission-gated')}</strong> — ${t('the agent must pass your Credential Vault permissions before using email tools')}</li>
        <li><strong>${t('Activity log')}</strong> — ${t('every agent email action is recorded in a local SQLite audit log')}</li>
        <li><strong>${t('Revocable')}</strong> — ${needsAppPw ? t("revoke the app password in your provider's security settings at any time") : t('change your password to instantly revoke access')}</li>
      </ul>
    </div>
  `;
...
export async function saveMailImapSetup(): Promise<void> {
  ...
  if (!email) {
    showToast(t('Email address is required'), 'error');
    return;
  }
  if (!password) {
    showToast(t('Password is required'), 'error');
    return;
  }
  if (!imapHost) {
    showToast(t('IMAP server is required'), 'error');
    return;
  }
  if (!smtpHost) {
    showToast(t('SMTP server is required'), 'error');
    return;
  }
  ...
  if (saveBtn) {
    ...
    saveBtn.textContent = t('Connecting...');
  }
  ...
  try {
    ...
    logCredentialActivity({
      accountName,
      action: 'approved',
      detail: `${t('Account connected')}: ${email} (${t('permissions')}: ${permList})`,
    });

    showToast(`${email} ${t('connected! Your agent can now read and send emails.')}`, 'success');
    closeChannelSetupFn?.();

    _loadMail();
  } catch (e) {
    showToast(`${t('Failed to connect')}: ${e instanceof Error ? e.message : e}`, 'error');
  } finally {
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.textContent = t('Connect Account');
    }
  }
}
